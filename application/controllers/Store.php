...
// require CartController "extends/"
...
class Store extends MY_Controller {
	...
	function __construct($params = array()) {
		...
		// Load product template and reference model
		$this->load->model("Product_insurcore_references_model");

		// Load Insurplatform library
		$this->load->library("Insurplatform/Insur_Cart", null, "insur_cart");
	...
	#region Order Management Service
	/**
	 * Adding parent cart item for sales product in product model.
	 * Save CartId into session by key: insur_cart_id.
	 * Note: with parentCartItem: productCode and productName will be based on policyTemplate (Order Management Service will do this).
	 * DeliverableItem need to fill out productCode, productName, bundleOptionCode, bundleOptionName
	 */
	private function sendAddParentItemCommand($data) {
		$bodyData = [
			"parentItemId" => null,
			"policyTemplateId" => $data["product_insurcore_references"]->policy_template_id,
			// with parentCartItem: productCode and productName will be based on policyTemplate (Order Management Service will do this)
			// note: deliverableItem need to fill out productCode, productName, bundleOptionCode, bundleOptionName
			// "productCode" => $data["product_insurcore_references"]->sales_product_code,
			// "productName" => $data["product_insurcore_references"]->sales_product_name,
			"productModel" => "SalesProduct",
			// "bundleOptionCode" => "",
			// "bundleOptionName" => "",
			"quantity" => 1,
			"attributeValues" => []
		...
		$userId = $user ? $user['id'] : "";
		$result = $this->insur_cart->addParentItem($userId, json_encode($bodyData));
		$responseData =json_decode($result["data"]);

		if (empty($result["error"])) {
			log_message("debug", get_class($this) . ": Insur_Cart - Result obtained");

			if ($result["info"]["http_code"] === 200 && (int)json_decode($result["data"])->code === 200) {
				$data["cart_item"] = json_decode($result["data"])->data;
			} else {
				log_message("error", get_class($this) . ": Insur_Cart - Http code error: " . $result["info"]["http_code"]);
				log_message("error", get_class($this) . ": Insur_Cart - Http data error: " . $result["data"]);
				log_message("error", get_class($this) . ": Insur_Cart - Body data sent: " . json_encode($bodyData));
			}
		} else {
			log_message("error", get_class($this) . ": Insur_Cart-Error occured: " . $result["error"]);
		}
	 ...
	 *
	 */
	private function sendAddDeliverableItemsCommand($cartId, $bodyData) {
		$user = $this->cart->is_logged();
		$userId = $user ? $user['id'] : "";
		$result = $this->insur_cart->addDeliverableItems($cartId, $userId, json_encode($bodyData));
		...
		return json_decode($result["data"])->data;
	}

	/**
	 * Sending more data to Order Management Service to update: shipping, customer, promotion, cart item's attributeValues.
	 *
	 */
	private function sendPreCheckoutCommand($insuranceExtraData) {
		//Call PreCheckout to order management service
		$user = $this->cart->is_logged();
		$result = $this->insur_cart->preCheckout($insuranceExtraData['cartId'], $user['id'], json_encode($insuranceExtraData));
		if (empty($result["error"])) {
			log_message("debug", get_class($this) . ": Insur_Cart - Result obtained");
			...
			if ($result["info"]["http_code"] === 200 && (int)json_decode($result["data"])->code === 200) {
				...
				log_message("error", get_class($this) . ": Insur_Cart - Http code error: " . $result["info"]["http_code"]);
				log_message("error", get_class($this) . ": Insur_Cart - Http data error: " . $result["data"]);
				log_message("error", get_class($this) . ": Insur_Cart - Body data sent: " . json_encode($insuranceExtraData));
			}
		} else {
			log_message("error", get_class($this) . ": Insur_Cart-Error occured: " . $result["error"]);
		}
	...
	/**
	 * send place order command to Order Management service
	 * @return result with orderId from Order Management service
	 */
	private function sendPlaceOrderCommand($insuranceExtraData, $shopBackUrl) {
		$user = $this->cart->is_logged();
		$result = $this->insur_cart->placeOrder($insuranceExtraData['cartId'], $insuranceExtraData['refId'], $user['id'], $shopBackUrl, $insuranceExtraData['policyTemplateCode']);
		if (empty($result["error"])) {
			log_message("debug", get_class($this) . ": Insur_Cart - Result obtained");
	...
	public function calculate_cart_unit_price() {
		$post = json_decode(file_get_contents('php://input'), TRUE);
		// $post = json_decode(file_get_contents('php://input'), true);
		$insuranceExtraData_deliverableItems = $post; // this object must contain cartItems and parentAttributeValues
		$insurCarId = $this->session->userdata('insur_cart_id');
		// $data = [
		// 	"parentAttributeValues" => [],
		// 	"cartItems" => $insuranceExtraData_deliverableItems
		// ];
		$deliverableItems = $this->sendAddDeliverableItemsCommand($insurCarId, $insuranceExtraData_deliverableItems);
		$json['cart_items'] = $deliverableItems;
		echo json_encode($json);
	}

	/**
	 * Retrieve Order's payment data.
	 * Case: order retrieved payment: return data will be contain payUrl
	 * @return result with payment from Order Management service
	 */
	private function retrieveOrdersPaymentData($orderId) {
		$result = $this->insur_cart->retrieveOrdersPaymentData($orderId);
		if (empty($result["error"])) {
			log_message("debug", get_class($this) . ": Insur_Cart - Result obtained");
			...
			if ($result["info"]["http_code"] === 200 && (int)json_decode($result["data"])->code === 200) {
				...
				log_message("error", get_class($this) . ": Insur_Cart - Http code error: " . $result["info"]["http_code"]);
				log_message("error", get_class($this) . ": Insur_Cart - Http data error: " . $result["data"]);
				log_message("error", get_class($this) . ": Insur_Cart - Body data sent: orderId: " .$orderId);
			}
		} else {
			log_message("error", get_class($this) . ": Insur_Cart-Error occured: " . $result["error"]);
		}
	...
	#end-region Order Management Service
	...
	public function index($user_id = 0, $store_slug = null){
		...
		else if(!empty($productionstore) && isset($productionstore['id']) && !empty($productionstore['id']))
		{
			...
			$data['settings'] = $this->Product_model->getSettings('store');
			log_message('error','HOME DATA ELSE');
			log_message('error', json_decode($data['settings']));
	...
	public function generateMustacheProductListData($products, $user_id){
		...
		foreach ($products as &$product) {
			...
			$product['product_suggested_price'] = (!empty($product['product_suggested_price'])) ? c_format($product['product_suggested_price']) : c_format($product['0']);

			$title_suffix = (strlen($product['product_name']) > 150) ? "..." : "";
			$product['product_name'] = mb_substr($product['product_name'], 0, 150).$title_suffix;
	...
	public function product($affiliate_id = 0, $product_slug) {
		...
		// !Clear user's cart information: because we allow customers to buy 1 insurance product at a time.
		$this->cart->clearCart();
		...
		if($product_slug){
			...
			if(!$data['product']){
				...
				// Load product insurcore reference
				$data["product_insurcore_references"] = $this->getProductInsurcoreReference($data["product"]["product_id"]);

				// Load sale product & policy template information
				$data["cart_item"] = null;

				// This is not a default product
				if ($data["product_insurcore_references"]) {
					$responseData = $this->sendAddParentItemCommand($data);
					$data["cart_items"] = $responseData->data;
					// temp save cartId from Insurplatform into sesion
					$this->session->set_userdata('insur_cart_id', $responseData->data->cartId);
					$this->session->set_userdata('insur_cart_parent_cart_id', $responseData->data->id);
				}
	...
	public function getProductInsurcoreReference($product_id) {
		return $this->Product_insurcore_references_model->getByProductId($product_id)[0];
	}
	...
	public function add_to_cart() {
		...
		if($product){
			...
			$product_insurcore_references = $this->getProductInsurcoreReference($product['product_id']);
			// if product is insurance sales product, we need to prevent allowVariationVersion
			$allowVariationVersion = $product_insurcore_references == null;
			$json['data'] = $this->cart->add($product['product_id'],$quantity,$variation,$this->cart->getReferId(), $product, $allowVariationVersion);
		} else {
			log_message("error", "add_to_cart - Error occured");
			log_message("error", "add_to_cart - Cannot find product id: " . $this->input->post("product_id", true));
		}
	...
	public function confirm_order() {
		...
		// more data for Insurance-Domain-Systems
		// $deliverableItems = json_decode($data['insuranceExtraData_deliverableItems'], true);
		$extraData = json_decode($data['extraData'], false);
		if ($extraData == null) {
			$json['errors'] = "insuranceExtraData's items is null";
		}

		$user = $this->cart->is_logged();
		$productList = $this->cart->getProducts();
		log_message("error", json_encode($products));
		$productInsurcoreReferences = $this->Product_insurcore_references_model->getByProductId($productList[0]['product_id']);
		$policyTemplateCode = $productInsurcoreReferences[0]->policy_template_code;
		log_message("error", "Policy Template Code => ");
		log_message("error", json_encode($policyTemplateCode));
		$insuranceExtraData = [
			"cartId" => $this->session->userdata('insur_cart_id'),
			// "storeId" => $this->cart->getcookieAffiliate(),
			"storeId" => "",
			"sellerId" => strval($this->cart->getReferId()),
			"customerEmail" => $user['email'],
			"customerName" => $user['lastname'].' '.$user['firstname'],
			"customerPhone" => $user['phone'] == null || $user['phone'] == "" ? "not_provided" : $user['phone'],
			"shippingAddress" => $user['address1'],
			"vourcherCode" => "",
			"paymentMethod" => "",
			"paymentProvider" => "",
			"items" => $extraData->insuranceExtraData_deliverableItems,
			"policyTemplateCode" => $policyTemplateCode
		];

		$paymentGateways = $this->session->userdata('payment_gateways');
		if ($paymentGateways == null) {
			log_message("error", "confirm_order - cannot retreive payment gateways");
		}

		$settingData = $paymentGateways[$data['payment_gateway']];
		// $settingData = $this->Product_model->getSettings($insuranceExtraData['paymentProvider']);
		$insuranceExtraData['paymentProvider'] = strtoupper($data['payment_gateway']);
		$insuranceExtraData['storeId'] = $settingData['store_id'];
		// isPaymentOnWebsite tạm thời sẽ hard bên order management
		// $insuranceExtraData['isPaymentOnWebsite'] = $settingData['is_payment_on_website'];
		...
		if(!empty($user) && !isset($json['errors']) && !isset($json['error'])){
			...
			if(!isset($json['errors']) && !isset($json['error'])){
				...
				// TODO:
				// !Must review product in cart before confirm order
				// !Must ensure there is one product in cart for every order
				$cart_product = $this->cart->getProducts();
				...
				foreach ($cart_product as $key => $product) {
					...
					if (
						((int)$referlevelSettings['status'] == 2 && in_array($product['refer_id'], $disabled_for)) ||
						in_array($product['refer_id'], $restricted_vendors) || $_refer_id > 0
					) {
						$refer_status = false;
					}
				...
				if($check_total_for_skip_payment>0) {
					...
					if($paymentGateways && isset($paymentGateways[$data['payment_gateway']])) {
						...
						// save more data for Insurance-Domain-Systems
						$insert['content']= serialize($order).' || '.serialize($products).' || '.serialize($insuranceExtraData);
						...
						$data['default_comment'] = 'Place Order';
						...
						log_message("error", "confirm_order - cannot retreive payment gateways before completing confirm order");
						log_message("error", json_encode($paymentGateways));
					}
				} else {
					$this->session->set_userdata('payment_gateway','cod');
					//$this->session->set_userdata('payment_gateways','');
	...
	public function payment_confirmation()
	{
		...
		if(!$json['errors'])
		{
			...
			$insuranceExtraData = unserialize($content[2]);

			//Call PreCheckout to order management service
			$this->sendPreCheckoutCommand($insuranceExtraData);
			$uncompleted['content'] = serialize($order).' || '.serialize($products).' || '.serialize($insuranceExtraData);
	...
	public function confirm_payment(){
		...
		if($paymentGateways && isset($paymentGateways[$paymentGateway])){
			...
			// // load created OrderId from Order Management Service into ExtraData.
			// // OrderId can be used at requesting payurl.
			// $insurOrderId = $this->sendPlaceOrderCommand($insuranceExtraData);
			// $insuranceExtraData['orderId'] = $insurOrderId->data;
			$uncompleted['content'] = serialize($order).' || '.serialize($products).' || '.serialize($insuranceExtraData);
			// $this->db->update('uncompleted_payment',$uncompleted,array('id' => $this->session->userdata('uncompleted_id')));
			...
			if((int)$paymentGateways[$paymentGateway]['setting_data']['proof'] == 1 && !$file){
				...
				if(in_array(strtolower($extension), $allow_ext)){
					...
					$uncompleted['content'] = serialize($order).' || '.serialize($products).' || '.serialize($insuranceExtraData).' || '.serialize($name);
					$this->db->update('uncompleted_payment',$uncompleted,array('id' => $this->session->userdata('uncompleted_id')));
				} else {
			...
			if(!isset($json['errors'])){
				if($this->session->userdata('uncompleted_id')){
					...
					// setPaymentGatewayRequest: do inserting order into database
	...
	public function confirmPaymentGateway($uncompleted_id, $status, $transaction_id = '', $comment = ''){
		...
		if($object->title){
			...
			$proofIndex = 3; //with base coding, we have proofIndex is 2. But aftering added insuranceExtraData into content of uncompleted_payment. We have a new index is 3
			$proof = unserialize($content[$proofIndex]);
			if($proof){
			...
			// OrderId can be used at requesting payurl.
			$insuranceExtraData = unserialize($content[2]);
			$insuranceExtraData['refId'] = $order_id;
			$shopBackUrl = base_url('store/thankyou/'.$uncompleted_id);
			$insurOrderId = $this->sendPlaceOrderCommand($insuranceExtraData, $shopBackUrl);
			$insuranceExtraData['orderId'] = $insurOrderId->data;
			$uncompleted['content'] = serialize($order).' || '.serialize($products).' || '.serialize($insuranceExtraData);
			// $this->db->update('uncompleted_payment',$uncompleted,array('id' => $this->session->userdata('uncompleted_id')));
				...
				foreach($products as $key => $product) {
					...
					if($product['admin_commission'] > 0 && $vendors['admin_sale_status'] == 1){
						$comission_data_batch[] = array(
							'status'       => (int)$status == 1 ? 1 : 0,
							'user_id'      => 1,
							'amount'       => $vendors['admin_commission_value'],
	...
	public function thankyou($uncompleted_id)
	{
		...
		// this is a trail action
		// TODO: clear this action
		$checkPayUrl = false;
		if ($checkPayUrl) {
			$insuranceExtraData = unserialize($content[2]);
			$paymentData = $this->retrieveOrdersPaymentData($insuranceExtraData['orderId']);
			$responseData =json_decode($paymentData["data"]);
		...
		if($data['order']['user_id'] == $user['id'] || $data['order']['user_id'] == base64_decode($_SESSION['guest'])){
			$data['settings'] = $this->Product_model->getSettings('store');
	...
	public function profile(){
		...
		if($post){
			...
			$this->form_validation->set_rules('Zip', 'Pincode');
			$this->form_validation->set_rules('twaddress', 'twaddress');
			...
			$this->session->set_flashdata('success',__('Cập nhật thành công'));
			redirect('store/profile');
		}
	...
	public function change_password(){
		...
		if($post){
			...
			$this->form_validation->set_message('required', 'Không thể để trống');
			$this->form_validation->set_message('trim', 'Không thể để trống');
			// set rules form
			$this->form_validation->set_rules('current_password', 'Mật khẩu hiện tại', 'required|trim');
			$this->form_validation->set_rules('new_password', 'Mật khẩu mới', 'required|trim|differs[current_password]',array('differs' => '%s không thể trùng với mật khẩu hiện tại'));
			$this->form_validation->set_rules('c_password', 'Mật khẩu nhập lại', 'required|matches[new_password]',array('matches' => '%s phải trùng với mật khẩu mới'));

			// set initdata form
			$this->session->set_flashdata('current_password', $this->input->post('current_password',true));
			$this->session->set_flashdata('new_password', $this->input->post('new_password',true));
			$this->session->set_flashdata('c_password', $this->input->post('c_password',true));
			if($this->form_validation->run() == FALSE){
				$this->session->set_flashdata('error',$this->form_validation->error_array());
				if($current_password && sha1($current_password) !== $user['password']){
					$this->session->set_flashdata('err', 1);
				}
				redirect('store/change_password');
			}
			if($current_password && sha1($current_password) !== $user['password']){
				$this->session->set_flashdata('err',1);
				redirect('store/change_password');
		...
		$data['category_tree'] = $this->Product_model->getCategoryTree();
		$data['category'] = $category;
		$this->storeapp->view("change_password", $data);
	}
	...
	public function order(){
		...
		$data['status'] = $this->Order_model->status();
		$data['class_class'] = $this->Order_model->statusClass();
		$data['class_icon'] = $this->Order_model->statusIcon();
		...
		$user = $this->getUser($userdetails['id']);
		$data['userDetails'] = array(
			'type'		  =>	$user['type'],
	...
	public function retrievepayment($uncompleted_id)
	{
		$data['uncompleted_id'] = $uncompleted_id;
		$_SESSION['guest_user'] = null;
		if(isset($_SESSION['guestFlow'])) unset($_SESSION['guestFlow']);
		if(isset($_SESSION['guestFlowClassified'])) unset($_SESSION['guestFlowClassified']);
		$uncompletedData = $this->Product_model->getByField('uncompleted_payment','id',$uncompleted_id);
		...
		if($data['order']['user_id'] == $user['id'] || $data['order']['user_id'] == base64_decode($_SESSION['guest'])){
			...
			$data['order_history'] = $this->Order_model->getHistory($order_id, 'order');
			$data['paymentsetting'] = $this->Product_model->getSettings('paymentsetting');
			$this->storeapp->view('checkout_retrievepayment', $data,true);
		} else {
			die(__('store.are_not_allow_to_see'));
		}
	}

	public function retrieve_orders_payment_data($uncompleted_id)
	{
		$uncompletedData = $this->Product_model->getByField('uncompleted_payment','id',$uncompleted_id);
		$content = explode(' || ',$uncompletedData['content']);
		$insuranceExtraData = unserialize($content[2]);
	...
	public function lookup_policy($code = ''){
		if(!empty($code)){
			$policyData = $this->sendLookupCodeCommand($code);
			$data['policy_detail'] = $policyData->data;
			$data['lookupcode'] = $code;
			log_message("error", "Data empty");
			log_message("error", json_encode($policyData));
			if(!empty($policyData->error)){
		...
		$this->storeapp->view('lookup_policy', $data);
	}

	public function get_policies($code = ''){
		if(!empty($code)){
	...
	private function sendLookupCodeCommand($lookupcode) {
		$result = $this->insur_cart->getPolicyByLookupCode($lookupcode);
		...
		if (empty($result["error"])) {
			...
			if ($result["info"]["http_code"] === 200 && (int)json_decode($result["data"])->code === 200) {
				// $responseData = json_decode($result["data"])->data;
			} else {
				log_message("error", get_class($this) . ": Insur_Cart - Http code error: " . $result["info"]["http_code"]);
				log_message("error", get_class($this) . ": Insur_Cart - Http data error: " . $result["data"]);
				log_message("error", get_class($this) . ": Insur_Cart - Body data sent: " . json_encode($bodyData));
