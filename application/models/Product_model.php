...
class Product_model extends MY_Model{
    ...
    /**
     * @description: This function has the same functionality with duplicateProduct function. But this is a custom version, that includes duplicate more data
     */
    public function duplicateProductWithInsurcoreReference($product_id, $status_review = false){
        $product = $this->db->query("SELECT * FROM product WHERE product_id=". (int)$product_id)->row_array();
        ...
        // Duplicate product insurcore reference data
        $product_insurcore_reference = $this->db->query("select * from product_insurcore_references where product_id=". (int)$product_id)->result()[0];
        if ($product_insurcore_reference) {
            unset($product_insurcore_reference->id);
            $product_insurcore_reference->product_id = $new_product_id;
            $this->db->insert("product_insurcore_references", $product_insurcore_reference);
        }
...
public function onlineCount(){
   ...
   function getAllProduct($user_id, $user_type, $filter = array()){
    ...
    if (isset($filter['is_campaign_product']) && !empty($filter['is_campaign_product'])) {
...
public function getAllUsersTreeV2ForUser($user_id, $first_time = true){
    ...
    public function calcCommitions($product, $type= 'sale', $shareUser = []){
        ...
        // $product_price = ((int)$product['quantity'] * (float)$product['product_price']); // original code
        $product_price = ((int)$product['quantity'] * ((float)$product['product_price'] + (float)$product['variation_price'])); //+ variation_price
...
public function getMyUnder($user_id, $first_time = true){
    ...
    if($this->level_count > $max_level) return array();
