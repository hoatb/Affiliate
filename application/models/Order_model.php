...
class Order_model extends MY_Model{
    ...
    public function statusClass($find=null) {
        $status = [
            '12'  => 'waiting_for_payment',
    ...
    public function statusIcon($find=null) {
        $status = [
            '12'  => 'bi bi-cash-coin',
            '0'  =>  'bi bi-cash-coin',
            '1'  =>  'bi bi-check2-circle',
            '2'  =>  'bi bi-x-square',
            '3'  =>  'bi bi-slash-circle-fill',
            '4'  =>  'bi bi-clock',
            '5'  =>  'bi bi-slash-circle-fill',
            '6'  =>  'bi bi-hourglass-top',
            '7'  =>  'bi bi-bootstrap-reboot',
            '8'  =>  'bi bi-cash-stack',
            '9'  =>  'bi bi-cash',
            '10' =>  'bi bi-x-circle',
            '11' =>  'bi bi-slash-circle'
        ];
        if($find!=null) {
            return $status[$find];
        } else {
...
public function getOrders($filter = array(), $addShipping = true){
    ...
    $query = "
        ...
        left join categories cate on cate.id = pc.category_id
    ...
    if(!isset($filter['page'])){
        ...
        foreach ($orders as $key => $value) {
            ...
            $product_slug = base_url("store/". base64_encode( (int)$pvalue->refer_id )  ."/product/". $value['product_slug']);
            $total_sum = $addShipping ? ($value['total_sum'] + $value['shipping_cost'] + $value['tax_cost']) : $value['total_sum'];
            $product_featured_image = resize('assets/images/product/upload/thumb/'. $value['product_featured_image'] , 100,100);
            $data[] = array(
                'id'                 => $value['id'],
                'status'             => $value['status'],
                'wallet_status'      => $value['wallet_status'],
