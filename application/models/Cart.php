...
class Cart extends CI_Model {
	...
	public function add($product_id,$quantity,$variation = null,$refer_id = 0, $product = false, $allowVariationVersion = true){
		$this->session->unset_userdata('last_order_id');

		$refer_id = (int)$refer_id;
		$session_id = $this->session->session_id;
		
		if ($allowVariationVersion) {
			$this->addProductWithVariationVersions($product_id, $product, $quantity, $variation, $session_id, $refer_id);
		} else {
			$this->addProductWithReplacingVariation($product_id, $product, $quantity, $variation, $session_id, $refer_id);
		}
	 ...
	 */
	private function addProductWithVariationVersions($product_id, $product, $quantity, $variation, $session_id, $refer_id) {
		$cart_product = $this->db->query("SELECT * FROM cart WHERE product_id  = {$product_id} AND product_variation = '{$variation}' AND session_id = ". $this->db->escape($session_id) ." ")->num_rows();
	...
	/**
	 * This is extend function for integration with Insur-platform.
	 * Preventing cart item with specific Variations. We only keep one product's Variation.
	 */
	private function addProductWithReplacingVariation($product_id, $product, $quantity, $variation, $session_id, $refer_id) {
		...
		if($cart_product == 0){
			...
			$this->db->query("UPDATE cart SET quantity = 1, product_variation = '{$variation}', refer_id={$refer_id} WHERE session_id = ". $this->db->escape($session_id));
		}
	}
	...
	public function getSelectedVariationPrice($product){
		...
		$price = 0;
		$foundRootProductVariationPrice = false;
		foreach($selected_var as $var_key => $var_value) {
			...
			foreach($product_var->{$var_key} as $p_var) {
				if($p_var->name == $searchVal) {
					$price += $p_var->price;
					$foundRootProductVariationPrice = true;
					break;
				}
			}
			// if ($foundRootProductVariationPrice == false)
			// {
			// 	$price += $var_value->price;
			// }
		}
		if ($selected_var && !$foundRootProductVariationPrice)//customer product_variations for Insurance platform
			$price += (int)$selected_var->price;
		return $price;
	}
